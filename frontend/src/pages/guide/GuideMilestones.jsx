...
const GuideMilestones = () => {
  ...
  const handleAction = async (id, action) => {
    try {
      const status = action === 'approve' ? 'GUIDE_APPROVED' : 'REJECTED';
  ...
  const pastMilestones = milestones.filter(m => m.status !== 'SUBMITTED');

  return (
    <div className="dashboard-page">
      <Helmet><title>Milestones - ProjectHub</title></Helmet>
      <div className="dashboard-header-text">
        <h1>Team Milestones</h1>
        <p>Review and approve milestones submitted by your teams.</p>
      </div>

      <Card className="dash-section-card" style={{ marginBottom: '2rem' }}>
        <h3 style={{ marginBottom: '1rem', color: 'var(--warning)' }}>Pending Review ({pendingMilestones.length})</h3>
        {pendingMilestones.length === 0 ? (
          <p style={{ textAlign: 'center', color: 'var(--text-secondary)', padding: '2rem' }}>No milestones pending review.</p>
        ) : (
      ...
      <Card className="dash-section-card">
        <h3 style={{ marginBottom: '1rem' }}>All Milestones</h3>
